...
## 2026-04-27 — feature/explore_sort 브랜치 머지 — 정렬 엔진 고도화, API 프록시화, 위시리스트 연동 강화

### 1. `origin/feature/explore_sort` 브랜치 머지 — 정렬 및 탐색 시스템 통합

**배경**: 팀원이 작업한 `feature/explore_sort` 브랜치의 정렬 기능 및 탐색 로직을 로컬 `doyeon` 브랜치에 풀+머지. `server/index.js`, `src/api/travelInfoApi.js`, `src/pages/Festivals.jsx`, `src/pages/TravelDetail.jsx` 등 7개 파일에서 충돌 해결.

**핵심 변경 내용 (`server/index.js`)**:
- **정렬 캐시 사전 계산 추가**: 서버 캐시 초기화 시 `createdtime`, `modifiedtime` 기준 4종(오름/내림차순) 정렬 결과를 미리 계산하여 `sortedTravelItems` 객체에 저장. API 요청 시 재정렬 없이 즉시 반환으로 성능 향상.
  ```javascript
  ...
  ```
- **축제 데이터 오늘 날짜 기반 필터링**: `eventStartDate`를 오늘 날짜(`new Date().toISOString().slice(0, 10).replace(/-/g, '')`)로 동적 설정하여 이미 종료된 행사를 서버 레벨에서 사전 제외.

**핵심 변경 내용 (`src/api/travelInfoApi.js`)**:
- **TourAPI 프록시 경유 구조 전환**: 모든 상세 정보 호출(`detailCommon2`, `detailIntro2`, `detailInfo2`, `detailImage2`)을 `/api/travel/proxy/:service` 엔드포인트 경유로 전환하여 클라이언트의 직접 TourAPI 호출 차단. 서버 측 프록시 캐시(2시간) 및 회로 차단기(429 발생 시 자동 차단) 활용.

**핵심 변경 내용 (`src/pages/TravelDetail.jsx`)**:
- **부가정보 항목(`infoItems`) 표시**: 상세 소개 정보를 화면에 렌더링하도록 수정.
- **지도 초기 렌더링 위치 오류 수정 (Bug Fix)**: 상세 페이지 첫 진입 시 지도가 엉뚱한 위치를 표시하던 문제 수정.

**핵심 변경 내용 (`src/pages/Explore.jsx`)**:
- **여행지 목록 정렬 기능 추가**: `useExploreStore`에 정렬 옵션(createdtime/modifiedtime × asc/desc) 상태 추가. 탐색 페이지 UI에 정렬 드롭다운 반영.

---

### 2. 축제 API 수정 — `/searchFestival2` 단독 사용 (Bug Fix)

**현상**: 서버 캐시에 `allTravelItems` 필터링 결과와 `festivalList2` 직접 호출 결과가 혼재하여 데이터 중복 및 정렬 불일치 발생.

**원인**: 축제 데이터 수집 시 두 소스(`areaBasedList2` 기반 필터 + `festivalList2` 직접 호출)가 병합되어 동일 행사가 중복 포함되는 경우 발생.

**수정 내용**:
- `server/index.js`에서 축제 정보를 `/searchFestival2` API 단독으로 수집하도록 변경.
- `eventStartDate`에 오늘 날짜를 동적으로 주입하여 만료된 행사를 API 레벨에서 원천 제외.

---

### 3. 위시리스트 폴더 날짜 타임존 오류 수정 (Bug Fix)

**현상**: 폴더에 날짜(예: 2026-04-27)를 설정하면 실제 화면에 하루 앞당겨진 날짜(2026-04-26)가 표시됨.

**원인 분석**: JavaScript의 `new Date('YYYY-MM-DD')`는 UTC 자정으로 파싱되어 한국(UTC+9) 로컬 환경에서 렌더링 시 전날이 됨. 서버에서도 `DATE` 컬럼이 ISO 문자열로 직렬화되는 과정에서 타임존 오프셋이 누적됨.

**수정 내용**:
- **`server/index.js`**: MySQL 연결 설정에 `dateStrings: true` 추가. 날짜 컬럼 조회 시 `DATE_FORMAT(start_date, '%Y-%m-%d')` 쿼리를 사용하여 문자열로 고정 전송.
- **`src/pages/MyPage.jsx`**: 날짜 표시 유틸리티(`parseLocalDate`, `formatScheduleShort`, `formatScheduleFull`)를 UTC 안전 로직으로 갱신. `.slice(0, 10)` 전처리로 ISO 문자열·순수 `YYYY-MM-DD` 형식 모두 정상 처리.

---

### 4. 위시리스트 알림 복구 및 축제 위시리스트 기능 추가

**수정 내용**:
- **알림 메시지 복구**: 여행지 탐색, 상세 페이지, 축제 페이지에서 위시리스트 추가/삭제 시 성공 알림(`alert`)을 복구하여 사용자 피드백 강화.
- **축제 위시리스트 기능 추가** (`Festivals.jsx`): 각 축제 카드 우상단에 하트 버튼 및 폴더 선택 모달(`WishlistModal`)을 이식. 행사 데이터(contentTypeId: 15)도 위시리스트 폴더에 저장 가능하도록 개선.
  - 비로그인 클릭 시 로그인 유도 알림 출력.
  - 이미 찜한 상태에서 클릭 시 즉시 삭제 토글.
...
- `npm run dev:all` 명령어로 프론트/백엔드 동시 실행 환경 구축.
