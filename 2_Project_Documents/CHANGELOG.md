...
- **다중 키워드 매핑**: 날씨 코드별 단일 키워드를 배열 형태의 다중 키워드로 확장.
    - 맑음: ['풍경', '바다', '야경', '카페']
    - 비: ['박물관', '카페', '실내']
- **지능형 랜덤 추천**: 매핑된 키워드 배열 중 하나를 랜덤으로 선택하여 검색함으로써 추천 결과의 다양성 확보.
- **가독성 개선**: 메인 Hero 섹션의 텍스트 대비를 강화하여 배경 이미지와 관계없이 정보 식별이 용이하도록 UI 수정.
