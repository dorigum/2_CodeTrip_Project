...
const Home = () => {
  ...
  const fetchMainData = useCallback(async (locProv = '서울', locCity = '서울', lat = 37.5665, lon = 126.9780) => {
    console.log('[Home] Starting fetchMainData for:', locProv);
    try {
      setProvince(locProv);
      
      // 개별 호출로 전환하여 하나가 실패해도 나머지는 나오도록 함
      getPhotoList(null, 15).then(tops => {
        if (tops.length > 0) setTopImgList(tops);
      });

      getCityBasedPlaces(locProv, 10).then(near => {
        console.log('[Home] Nearby places fetched:', near.length);
        if (near.length > 0) setNearbyPlaces(near);
        setLoading(prev => ({ ...prev, nearby: false }));
      });

      getFestivalList(10).then(fests => {
        const festItems = (fests || []).slice(0, 3).map(f => ({
          type: 'festival', icon: 'celebration', title: f.title, 
          subtitle: f.eventstartdate || 'Info',
      ...
      getWeather(lat, lon).then(async (wData) => {
        setWeather({ ...wData, location: locCity });
        const recs = await getPhotoList(wData.keywords, 1);
        if (recs.length > 0) setWeatherRec(recs[0]);
        setLoading(prev => ({ ...prev, weather: false }));
      });

    } catch (err) {
      console.error('[Home] Fatal fetch error:', err);
    }
  }, []);

  useEffect(() => {
    console.log('[Home] useEffect mounted');
    // 1. 즉시 로딩 시작 (네트워크 요청 발생 보장)
    fetchMainData();
    ...
    if (navigator.geolocation) {
      navigator.geolocation.getCurrentPosition(
        async (pos) => {
          ...
          fetchMainData(locData.state, locData.city, pos.coords.latitude, pos.coords.longitude);
        },
        (err) => console.warn('[Home] Geolocation disabled:', err.message),
        { timeout: 5000 }
      );
    }
  }, [fetchMainData]);
  ...
  return (
    <div className="p-6 lg:p-10 space-y-6 flex-1 flex flex-col bg-background overflow-hidden max-h-screen">
          ...
          <div className="bg-white p-8 rounded-3xl shadow-xl border border-outline-variant/10 relative overflow-hidden flex flex-col group">
            {loading.nearby && (
              <div className="absolute inset-0 bg-white/90 z-20 flex flex-col items-center justify-center gap-2">
                ...
                <p className="text-[10px] font-mono text-outline uppercase animate-pulse">// fetching_nearby...</p>
              </div>
            )}
            <div className="flex-1 flex flex-col space-y-5">
              <div className="flex justify-between items-start">
                <div className="space-y-1">
                  <div className="flex items-center gap-2 text-primary font-bold">
                    ...
                    <p className="text-[10px] uppercase tracking-widest font-label">NEAR ME ({province} {weather.location})</p>
                  </div>
              ...
              <div className="h-52 w-full rounded-2xl overflow-hidden bg-slate-100 shadow-inner">
                <img 
                  src={nearbyPlaces[nearbyIndex]?.firstimage || 'https://images.unsplash.com/photo-1464822759023-fed622ff2c3b?q=80&w=2070'} 
                  className="w-full h-full object-cover transition-transform duration-1000 group-hover:scale-110" 
                  alt="n" 
                  onError={(e) => { e.target.src = 'https://images.unsplash.com/photo-1464822759023-fed622ff2c3b?q=80&w=2070'; }} 
                />
              </div>
              <div>
                <div className="flex items-center justify-between gap-2">
                  <h4 className="font-bold text-xl text-slate-900 truncate font-headline">
                    {nearbyPlaces.length > 0 ? nearbyPlaces[nearbyIndex]?.title : <span className="text-slate-300 italic text-sm font-mono">// no_travelInfo_found</span>}
                  </h4>
                ...
                <div className="flex items-center gap-2 mt-2 text-slate-400 font-mono text-xs italic">
                  ...
                  <p className="truncate">{nearbyPlaces.length > 0 ? nearbyPlaces[nearbyIndex]?.addr1 : `${province} 인기 명소 탐색`}</p>
                </div>
              </div>
            </div>
          </div>
            ...
            <div className="flex-1 flex flex-col space-y-5">
              <div className="flex justify-between items-start">
                <div className="space-y-1">
                  ...
                  <h3 className="font-headline text-2xl font-bold text-slate-900 truncate">🎲날씨 기반 여행지 뽑기: {weather.label}</h3>
                </div>
              ...
              <div className={`h-52 w-full rounded-2xl overflow-hidden bg-slate-100 relative shadow-inner ${isSlotSpinning ? 'scale-[1.02]' : ''}`}>
                <img 
                  src={weatherRec?.galWebImageUrl || 'https://images.unsplash.com/photo-1506744038136-46273834b3fb?q=80&w=2070'} 
              ...
              <div>
                <div className="flex items-center justify-between gap-2">
                  ...
                  {weatherRec && hasPicked && <Link to={`/explore/${weatherRec.galContentId}`} className="shrink-0 text-[10px] font-bold text-primary hover:underline uppercase tracking-widest bg-primary/5 px-3 py-1 rounded-full font-label">View_Detail</Link>}
                </div>
                <div className="flex items-center gap-2 mt-2 text-slate-400 font-mono text-xs italic"><span className="text-primary-container">//</span><p className="truncate">{weatherRec?.galPhotographyLocation || '현재 날씨와 어울리는 여행지 탐색'}</p></div>
              </div>
            </div>
          </div>
        </div>
